...
function LoginScreen({navigation}) {
  ...
  const handleLogin = async (values) => {
    ...
    console.log(values);
    try {
      const response = await fetch('https://petstore.swagger.io/v2/user/login?username='+values.email+'&password='+values.password, {
        method: 'GET',
        headers: {
      ...
      console.log(data);
      data.code === 200 ? navigation.navigate('MainHome') : console.log(data.code)
      // process data
    } catch (error) {
      console.log(error);
      // handle error
  ...
  const ActivityIndicators = () => (
   ...
   return (
      <ImageBackground source={require('../assets/pets.jpg')} style={styles.bgImage} resizeMode='cover'>
