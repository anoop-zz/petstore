...
const Filter = () => {
    ...
    const handleFilter = (itemValue) => {
      ...
      console.log('https://petstore.swagger.io/v2/pet/findByStatus?status=' + itemValue  );

      fetch('https://petstore.swagger.io/v2/pet/findByStatus?status='+ itemValue, {
    method: 'GET',
    headers: {
    ...
    return <VStack alignItems="center" space={4}>
        <Select placeholderTextColor={'black'} shadow={2} selectedValue={service} minWidth="200" accessibilityLabel="Filter" 
        placeholder="Filter" _selectedItem={{
        bg: "teal.600",
 ...
 const CartItem = ({item}) => (
  ...
  return  (
      <NativeBaseProvider>
        <Button title='Cart' onPress={() => navigation.navigate('CartScreen')} />
    ...
    />
       ...
       <FlatList
        ...
        />
        {data.length===0 && (<View style={styles.noPets}>
          <Text style={styles.petText}>No Pets Available</Text>
          <TouchableOpacity onPress={reloadApp}><Text style={styles.reload}> Reload Feed
